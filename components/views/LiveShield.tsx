...
export function LiveShield({ notify }: { notify: (m: string) => void }) {
  ...
  async function startStream() {
    ...
    if (!edgeUrl) {
      setVoiceState("error"); 
      setVoiceError("NEXT_PUBLIC_EDGE_WS_URL is not configured. Configure the voice edge WebSocket URL.");
      notify("Live capture requires NEXT_PUBLIC_EDGE_WS_URL to be configured.");
      return;
    }
