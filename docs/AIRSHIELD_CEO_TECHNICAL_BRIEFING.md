...
**Version 1.1 · 24 August 2026 · Internal hackathon briefing**
...
| EgressSeal™ | Ed25519-signed development seal bound to protected digest, destination, risk, receipt, expiry and action | Externally trusted KMS/HSM signer and authoritative upstream-receipt verifier |
| Destination Switch | Interactive route rebinding and recalculation across private, managed, research and public routes | Customer policy registry, live destination identity, revocation and network enforcement |
| ContextFence™ | Explainable cumulative-risk meter for linkage/mosaic risk after masking | Evaluated sector model, conversation memory, threshold governance and monitoring |
| SafeAction™ | Seal-gated synthetic connector with token-only broker path and signed action receipt | Separately deployed action broker and real least-privilege connectors |
...
## 11A. EgressSeal product layer

EgressSeal™ turns a protection result into a destination-bound release object. Instead of treating a green UI state as authorization, it binds the protected-content digest, upstream receipt reference, tenant policy, exact destination, ContextFence result, permitted action, issue time, expiry and nonce into one signed envelope. Changing content, destination or action invalidates the authorization.

The hackathon route uses a process-local upstream-receipt registry and Ed25519 key. The registry requires the receipt identifier, protected digest, decision, policy and destination created by `/api/protect` to match before signing. It then supports three operations:

1. **Issue:** refuse when upstream decision is not allow, receipt is missing, destination is blocked or ContextFence exceeds the destination threshold.
2. **Verify:** validate signature, signing-key identity, expiry and protected-content digest before unlocking downstream behavior.
3. **Execute:** verify the seal again, enforce the single policy-derived action, use token references only and produce a signed synthetic action receipt.

### Destination Switch

Destination is treated as security context, not a cosmetic dropdown. The same protected payload can be allowed for an organization-private AI, allowed with a narrower threshold for a managed regional RIA, held for a research sandbox or blocked for a public general AI. A switch discards the previous seal because an authorization for one destination must never be replayed to another.

### ContextFence

Traditional masking evaluates fields independently. ContextFence addresses the mosaic problem: multiple tokens, entity classes, semantic details, timing, measurements, relationships and long conversations can collectively identify a person even when no direct identifier remains. The demonstration produces an explainable score and destination-specific allow, review or block result.

ContextFence is a policy signal, not a mathematically proven anonymity guarantee. Production thresholds require representative evaluation, attack simulation, utility testing, governance and monitoring.

### SafeAction

SafeAction demonstrates a stronger alternative to giving an AI vault access. The model sees and proposes an operation using tokens. A trusted broker verifies the seal and allowlist, then resolves only inside the connector boundary. The action receipt states that raw values were not visible to the model.

The current connector is synthetic and resolves no real identity. Production requires a separately deployed broker, scoped service identity, idempotency store, connector-specific field policy, approval/escalation controls, real system-of-record integration and output protection.

The ™ marks denote product-concept branding, not registered trademark status. The development signer is cryptographically functional but not a production authority; production must use a pinned external KMS/HSM trust anchor and authoritative upstream-receipt verification.
